...
function updatePackageFile({ args, names, paths }) {
  try {
    ...
    const packageFile = require(paths.package);
    // If packageFile.scripts is not defined, define it as an empty object.
    ...
    packageFile.scripts.calliope = 'yarn install && calliope';
    packageFile.scripts.build = 'yarn calliope build';
    packageFile.scripts.lint = 'yarn calliope lint';
    packageFile.scripts.start = 'yarn calliope start';
    // Make the test script just be a wrapper for the lint script.
    packageFile.scripts.test = 'yarn lint';
